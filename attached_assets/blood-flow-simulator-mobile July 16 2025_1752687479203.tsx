...
const BloodFlowSimulator = () => {
  ...
  const getAdjustedPressures = (upstream, downstream, constriction) => {
  ...
  const getPressureData = () => {
    ...
    for (let i = 0; i <= steps; i++) {
      ...
      data.push({ position: position.toFixed(0), pressure });
    }
    return data;
  };
  ...
  return (
    <div className={`max-w-6xl mx-auto min-h-screen relative ${isMobile ? 'p-2' : 'p-4'}`}>
      <Card className={isMobile ? "mb-1" : "mb-4"}>
        ...
        <CardContent className={isMobile ? "p-2" : ""}>
          ...
          <div className="relative">
            <canvas ref={canvasRef} className={`w-full bg-gray-100 ${isMobile ? 'h-20' : 'h-40'}`} />
            {isMobile ? (
          ...
          {isMobile ? (
            <div className="text-center mt-1">
              ...
              <div className="text-xs">{flow.toFixed(2)} mL/min</div>
            </div>
          ) : (
            <div className="text-center mt-2">
              <div className="font-bold">Flow</div>
              <div>{flow.toFixed(2)} mL/min</div>
            </div>
          )}
        </CardContent>
      </Card>
      ...
      <div className={isMobile ? "flex flex-col gap-1 mb-1" : "flex gap-4 mb-2"}>
          ...
          <CardContent className={isMobile ? "p-2 space-y-1" : ""}>
            <div className={isMobile ? "mb-1" : "mb-4"}>
              <div className="flex justify-between items-center">
                <label className={`${isMobile ? 'text-xs' : 'block mb-2'}`}>
            ...
            <div>
              ...
              <Slider
                ...
                step={1}
              />
              <div className={`text-center mt-2 ${isMobile ? 'text-xs' : ''}`}>{constrictionPercent}% constricted</div>
            </div>
            {!isMobile && (
              <button 
        ...
        <div className={isMobile ? "w-full" : "w-3/5"}>
          ...
          <CardContent>
            <LineChart
              width={isMobile ? Math.min(windowWidth - 80, 300) : 320}
              height={isMobile ? 180 : 280}
              data={getPressureData()}
              margin={isMobile ? { top: 20, right: 20, bottom: 25, left: 20 } : { top: 20, right: 20, bottom: 40, left: 20 }}
            >
              ...
              />
              <Line type="monotone" dataKey="pressure" stroke="#8884d8" dot />
            </LineChart>
          </CardContent>
                  </Card>
        </div>
      ...
      <Card>
        ...
        <CardContent className={isMobile ? "pt-0 pb-0" : "pt-1 pb-2"}>
          <div className={`text-center ${isMobile ? 'text-[8px] leading-none' : 'text-lg'}`}>
            Flow = ΔP/R, where R ∝ 1/r⁴
          </div>
          <div className={`text-center ${isMobile ? 'text-[8px] leading-none mt-0' : 'text-sm mt-1'}`}>
